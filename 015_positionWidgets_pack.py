...
Il existe 3 méthodes de positionnement de widgets :
    -> la méthode pack() décrite ci-dessous avec l'instruction expand=1 (affichage sur toute la zone non occupée)
    -> la méthode grid()
    -> la méthode place()
...
btn.pack(expand=1)  # Affichage du bouton sur tout la zone non occupée par les autres widgets
