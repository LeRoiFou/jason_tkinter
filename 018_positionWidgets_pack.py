...
Il existe 3 méthodes de positionnement de widgets :
    -> la méthode pack() décrite ci-dessous avec les instructions pad et ipad : pour pad on détermine les marges
    externes du widget (donc la fenêtre va être dimensionnée avec ces marges) et pour ipad on détermine les marges
    internes du widget
    -> la méthode grid()
    -> la méthode place()
...
app.geometry()  # Dimension de la fenêtre
...
btn.pack(padx=100, ipady=5)  # Affichage du bouton
